...
export function AccrualJournalDetails({ journal, onUpdate }: AccrualJournalDetailsProps) {
  ...
  // Initialize store allocations if they don't exist
  const storeAllocations = React.useMemo(() => 
  ...
  // Check if we should be in multi-store mode
  React.useEffect(() => {
    if (journal.storeAllocations && journal.storeAllocations.length > 1) {
  ...
  const addStoreAllocation = () => {
    ...
    updateStoreAllocations([...storeAllocations, newAllocation]);
  };

  const removeStoreAllocation = (id: string) => {
    if (storeAllocations.length > 1) {
      updateStoreAllocations(storeAllocations.filter(allocation => allocation.id !== id));
    }
  };
